...
/*	if (argc < 4) {
	...
	//writeInt(result,ttNumber);
	fprintf(result,"%d\n",ttNumber);
	
	for (qq = 0; qq < ttNumber; qq++, ++aaa) {
    ...
    for (ix = 2 ; ix <= ptop ; ix++){
		...
		for (j = 1; j < atop; ++j) {
			...
			if (flag > -1) 
				//writeInt(result,flag);
				fprintf(result,"%d\t",flag);
			else 
				//writeInt(result,last[left]);
				fprintf(result,"%d\t",last[left]);
			//writeDouble(result,anst[j]);
			fprintf(result,"%lf\t",anst[j]);
			//printf("[%d %lf]\t",flag,anst[j]);
		}
