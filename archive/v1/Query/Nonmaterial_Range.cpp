...
double circleDistance(double lat1,double long1,double lat2,double long2)
{
	...
	double getStLen(int u, int v) {
	return circleDistance(coordNode[u << 1],coordNode[(u << 1)+1],coordNode[v << 1],coordNode[(v << 1)+1]);
}
...
int rangeQuery(int startT, int endT, double qx1,double qy1, double qx2,double qy2, int spNumber) {
	...
	for (int i = 0; i < spatialNumber; ++i) {
		...
		if (dist + getStLen(path[i], path[i+1]) <= d1) {
			dist += getStLen(path[i], path[i+1]); continue;
		}
		
		int ne = path[i];
		while (other[ne] != thisSide[path[i+1]]) {
			nodeToEdgeDistanceAndNodeSide(0,0,ne,&sideLen,&roadLen);
			if (dist + roadLen <= d1) {
				dist += roadLen; continue;
			}
			if (crossEdge(ne,qx1,qy1,qx2,qy2)) return 1;
			int nowStreet = road2Street[ne][0];	//起始道路
			int nowBias = road2Street[ne][1];	//起始偏移
			ne = street2road[nowStreet][nowBias + 1];
		}
	}
	return 0;
}
