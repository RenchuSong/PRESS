...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < tjNumber; ++i) {
		...
		/*
		for (j = 0; j < spNumber; ++j) if (valid[j]) printf("%d\t",j);puts("");
		...
		for (j = 1; j < spNumber; ++j) {
			int flag = 1;
			int k;
			for (k = preNode + 1; k < j; ++k) {
				double proDist = dist[preNode] + (double)(dist[j] - dist[preNode])*(timing[k] - timing[preNode]) / (timing[j] - timing[preNode]);
				double proDist2 = 0;
				if (dist[j] != dist[preNode]) {
					proDist2 = timing[preNode] + (double)(timing[j] - timing[preNode])*(dist[k] - dist[preNode])/(dist[j] - dist[preNode]);
					//printf("%lf\t",proDist2 - timing[k]);
					if (myabs(proDist2 - timing[k]) > tolerance2) {
						flag = 0; break;
					}
				}
				if (myabs(proDist-dist[k]) > tolerance) {
					flag = 0;break;
				}
			}
			if (!flag) {
