...
int rangeQuery(string code, int startT, int endT, double qx1,double qy1, double qx2,double qy2, int spNumber) {
	int s,e;
	double d1,d2;
	...
	double tdis;
	int trieNum;
	int trieTarget1,trieTarget2;
	int result1 = -1;
	int result2 = -1;
	int final_s1,final_t1,final_s2, final_t2;   
	int inSP = -1;
	bool mark1 = 0;
	bool mark2 = 0;
	vector<int> edge;
	vector<pair<int,int>> sp;
	double start_d1,start_d2;
	for(int i = 0;i < code.length();i++) {
		int cur = code[i]-'0';
		if(cur == 0)
			c = L[c];
...
//				cout << curD << endl;
			...
			//	cout << lastD << "\t" << curD << "\t 1"<< endl;
				...
				if(mark1 == 0 && curD > d1) {
					/*落在两段边之间的最短路径上*/
					final_s1 = last;
					final_t1 = thisSide[value[father2[map[c]]]];
					start_d1 = lastD;
	//				result1 = 0;
					mark1 = 1;
				}
				if(mark1 == 1) {
					sp.push_back(make_pair(last,thisSide[value[father2[map[c]]]]));
				}
				if(curD > d2) {
					final_s2 = last;
					final_t2 = thisSide[value[father2[map[c]]]];
					mark2 = 1;
	//				result2 = 0;
					start_d2 = curD;
					break;
	...
	//cout << edge.size() << "\t" << sp.size() << endl;
	for(int i = 0;i < sp.size();i++) {
		//cout << i << endl;
		double x1,x2,y1,y2;
		pathMBR(sp[i].first,sp[i].second,&x1,&x2,&y1,&y2);
		
		if(MBRmixed(x1,x2,y1,y2,qx1,qx2,qy1,qy2) == 0)
			continue;
		//cout << i << endl;
		//cout << i << "\t" << start_d2 << "\t" << d2 << endl;;
		if(i == 0 && start_d1 < d1) {
	//		cout << 1 << endl;
			start_d1 += pathLen(final_s1,final_t1);
			int ts = sp[i].first;
			int te = sp[i].second;
			while(ts != SPLINK[ts][te] && start_d1 > d1) {
				if(crossEdge(SPLINK[ts][te],qx1,qy1,qx2,qy2) == 1)
					return 1;
				start_d1 -= pathLen(thisSide[SPLINK[ts][te]],te);
				te = thisSide[SPLINK[ts][te]];
			}
			continue;
		}
		if(i == sp.size()-1 && start_d2 > d2) {
	//		cout << 2 << endl;
			int ts = sp[i].first;
			int te = sp[i].second;
			while(ts != te) {
			//	cout << ts << "\t" << te << endl;
				double td = pathLen(thisSide[SPLINK[ts][te]],te);
				if(start_d2-td < d2)
					break;
				start_d2 -= td;
				te = thisSide[SPLINK[ts][te]];
			...
			//cout << 1 << endl;
			while(ts != thisSide[SPLINK[ts][te]] && ts != te) {
				if(crossEdge(SPLINK[ts][te],qx1,qy1,qx2,qy2) == 1)
					return 1;
				te = thisSide[SPLINK[ts][te]];
			}
	...
	//	cout << 3 << endl;
		int ts = sp[i].first;
		int te = sp[i].second;
		while(ts != thisSide[SPLINK[ts][te]]) {
			if(crossEdge(SPLINK[ts][te],qx1,qy1,qx2,qy2) == 1)
				return 1;
			te = thisSide[SPLINK[ts][te]];
		}
	...
	for(int i = 0;i < edge.size();i++) {
		if(crossEdge(edge[i],qx1,qy1,qx2,qy2) == 1)
			return 1;
	}
	return 0;
}
int main(int argc, char *argv[]) {
	...
	for(int j = 0;j < queryNumber;j++) {
		...
		//if(j == 1052)
			...
			*/
			long src = clock();
			fprintf(oup,"%d",rangeQuery(spCode,TIME1,TIME2,RANGE[0][0],RANGE[0][1],RANGE[0][2],RANGE[0][3],pNumber));
			fprintf(oup,"%d",rangeQuery(spCode,TIME1,TIME2,RANGE[1][0],RANGE[1][1],RANGE[1][2],RANGE[1][3],pNumber));
			fprintf(oup,"%d",rangeQuery(spCode,TIME1,TIME2,RANGE[2][0],RANGE[2][1],RANGE[2][2],RANGE[2][3],pNumber));
			fprintf(oup,"%d",rangeQuery(spCode,TIME1,TIME2,RANGE[3][0],RANGE[3][1],RANGE[3][2],RANGE[3][3],pNumber));
			fprintf(oup,"%d\n",rangeQuery(spCode,TIME1,TIME2,RANGE[4][0],RANGE[4][1],RANGE[4][2],RANGE[4][3],pNumber));
			ttr += clock() - src;
		//system("pause");
	}
	fclose(oup);
