...
int ansSize;					//压缩后实体个数
int ansList[20000][3];			//压缩后的轨迹，测试解压用
double ans4[20000];
int preIndex[20000];
//============
...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < tjNumber; ++i) {
		...
		ansList[ansSize][0] = 2;ansList[ansSize][1] = path[preEdge];ansList[ansSize++][2] = spNumber - preEdge;
		
		for (j = 0; j < ansSize; ++j) {
			ans4[j] = 0;
			int k;
			for (k = 0; k < ansList[j][2]-1; ++k) {
				nodeToEdgeDistanceAndNodeSide(0,0,path[preIndex[j]+k],&sideLen,&roadLen);
				ans4[j] += roadLen;
			}
			if (j > 0) ans4[j] += ans4[j-1];
		}
		//nowStreet = road2Street[path[nowEdge]][0];	//道路
		//nowBias = road2Street[path[nowEdge]][1];	//偏移
		...
		for (j = 0; j < ansSize; ++j) {
			if (j > 0) ans4[j] += ans4[j-1];
			//printf("[%d %d]\t",ansList[j][1], ansList[j][2]);
			//writeInt(result,(15 << 28) | ((ansList[j][1] & 65535) << 8) | (ansList[j][2] & 255));
			writeInt(result,ansList[j][1]);
			writeDouble(result,ans4[j]);
		}
